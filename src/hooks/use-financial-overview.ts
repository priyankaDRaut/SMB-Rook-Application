...
export const useFinancialOverview = ({ 
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      // If no clinicId is provided, we can't make the API call
      if (!clinicId) {
      ...
      try {
        ...
        console.log('🔍 Fetching financial overview data for clinic:', clinicId);
        console.log('🔍 Date range:', { startDate: dateRange.startDate, endDate: dateRange.endDate });
        console.log('🔍 Environment:', import.meta.env.DEV ? 'Development' : 'Production');
        
        const result = await makeApiRequest(
          API_CONFIG.ENDPOINTS.FINANCIAL_OVERVIEW,
          accessToken,
          { 
            clinicId,
        ...
        console.log('✅ Financial overview API response:', result);
        
        setData(result);
        setIsUsingFallbackData(false);
      } catch (err) {
        const errorMessage = err instanceof Error ? err.message : 'Failed to fetch financial overview data';
        setError(errorMessage);
        console.error('❌ Financial overview API Error:', err);
    ...
    fetchData();
  }, [clinicId, effectivePeriod, startDate, endDate
